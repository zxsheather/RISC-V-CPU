...
class LSQ(Module):
    ...
    def build(
        ...
        with Condition(revert_flag_cdb[0]):
            ...
            with Condition(sq_pos_need_update):
                ...
                for i in range(LSQ_SIZE):
                    is_committed_sq = sq_is_committed_array_d[i][0] & sq_busy_array_d[i][0]
                    is_selected_sq = is_committed_sq & (sq_lsq_pos_array_d[i][0] > max_lsq_pos)
                    max_lsq_pos = is_selected_sq.select(sq_lsq_pos_array_d[i][0], max_lsq_pos)
                ...
                self.log(
                    ...
                    (max_sq_committed_index.bitcast(UInt(32)) + UInt(32)(1)).bitcast(Bits(32)),
                )
        ...
        load_for_store_flag = (
            store_flag & (sq_mem_oper_size_array[sq_head_pos] != Bits(2)(2)) & (~sq_is_waiting[0])
        )
        ...
        rob_dest_to_rob[0] = load_flag.select(lq_rob_dest_array[lq_head_pos], Bits(32)(0))
        ...
        with Condition(load_flag | store_flag):
            ...
            with Condition(load_flag):
                ...
                lq_head[0] = (lq_head[0].bitcast(Int(32)) + Int(32)(1)).bitcast(Bits(32)) & Bits(
                    32
                )(LSQ_SIZE - 1)
            ...
            with Condition(store_flag):
                with Condition(read_mux(sq_lsq_pos_array_d, sq_head[0]) == Bits(32)(0)):
                    ...
                    self.log("Store with lsq_pos=0, treating as committed already before flushing")
                with Condition(read_mux(sq_lsq_pos_array_d, sq_head[0]) != Bits(32)(0)):
                    lsq_head[0] = (lsq_head[0].bitcast(Int(32)) + Int(32)(1)).bitcast(Bits(32))
        ...
        with Condition(sq_is_waiting[0]):
            ...
            with Condition(read_mux(sq_lsq_pos_array_d, sq_head[0]) != Bits(32)(0)):
                lsq_head[0] = (lsq_head[0].bitcast(Int(32)) + Int(32)(1)).bitcast(Bits(32))
            write_1hot(sq_busy_array_d, sq_head[0], Bits(1)(0))
            sq_head[0] = (sq_head[0].bitcast(Int(32)) + Int(32)(1)).bitcast(Bits(32)) & Bits(32)(
                LSQ_SIZE - 1
            )
            mem_addr_to_rob[0] = sq_addr_array[sq_head_pos]
        ...
        wdata_half = offset[1:1].select(
            concat(store_val[0:15], rdata[0:15]), concat(rdata[16:31], store_val[0:15])
        )

        store_val = (size == Bits(2)(0)).select(
            wdata_byte, (size == Bits(2)(1)).select(wdata_half, store_val)
        )
