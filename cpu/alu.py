...
from .utils import ALULogEnabled, Logger
...
class ALU(Module):
    ...
    def build(self, value_to_rob: Array, index_to_rob: Array, valid_to_rob: Array):
        ...
        results[RV32I_ALU.ALU_ADD] = (alu_a.bitcast(Int(32)) + alu_b.bitcast(Int(32))).bitcast(
            Bits(32)
        )
        results[RV32I_ALU.ALU_SUB] = (alu_a.bitcast(Int(32)) - alu_b.bitcast(Int(32))).bitcast(
            Bits(32)
        )
        results[RV32I_ALU.ALU_SLL] = alu_a << alu_b[0:4]
        results[RV32I_ALU.ALU_SRA] = (alu_a.bitcast(Int(32)) >> alu_b[0:4].bitcast(Int(5))).bitcast(
            Bits(32)
        )
        ...
        results[RV32I_ALU.ALU_CMP_EQ] = (alu_a == alu_b).select(Bits(32)(1), Bits(32)(0))
        results[RV32I_ALU.ALU_CMP_LT] = (alu_a.bitcast(Int(32)) < alu_b.bitcast(Int(32))).select(
            Bits(32)(1), Bits(32)(0)
        )
        results[RV32I_ALU.ALU_CMP_LTU] = (alu_a < alu_b).select(Bits(32)(1), Bits(32)(0))
